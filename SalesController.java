...
public class SalesController implements Initializable {
   ...
   @FXML
    private JFXTextField eqdaterecv;

    @FXML
    private JFXTextField ENo1;

    @FXML
    private JFXComboBox<String> eqch;
 ...
 public void fetcheqnos(){//this function is used to fill the enquiry combo box with existing enquiry nos
    try{
         String enqfetch= "Select Eqno from enquiry where 1; ";
        PreparedStatement st = com.mycompany.snp.MainApp.conn.prepareStatement(enqfetch);
        ResultSet res = st.executeQuery();
        while(res.next()){
            String s=res.getString("Eqno");
            eqch.getItems().add(s);
        }
