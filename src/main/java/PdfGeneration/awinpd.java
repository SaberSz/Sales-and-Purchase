...
public class awinpd {
    ...
    public static String DEST = "results/Invoice/Awin/";

    public awinpd(HashMap<String, Object> hm) throws IOException {
    ...
    public void createPdf(String dest, HashMap<String, Object> hm) throws IOException {
        ...
        Paragraph address = new Paragraph().add(new Text(" AWIN ENGINEERING PTE LTD\n").setFont(PdfFontFactory.createFont("ttffiles/ARLRDBD.ttf"))
                .setFontColor(cuntz).setFontSize(12)).add(new Text("Works: No. 109, Tuas South Ave. 8, Singapore 637 037.\n"
                + "Regd. No. 12 Tuas View Place, Singapore 637 034.\n"
                + "Tel: +65 6778 8271, Fax: +65 6265 7685.\n"
                + " Email: vijay@awin.com.sg,  Web site: www.awin.com.sg")
                .addStyle(normal)).setFont(PdfFontFactory.createFont("arial-unicode-ms/ARIALUNI.TTF")).setFontSize(9.5f);//change path here 
        address.setFixedLeading(18);//line spacing
        address.setMarginLeft(15);//set text in paragraph a little to left
        Paragraph dets = new Paragraph().add(steelslogo).add(new Text("\nCo.Reg No: 201012187G\n" + "GST.Reg No: 201012187G").addStyle(cal)).setTextAlignment(TextAlignment.JUSTIFIED_ALL).setFont(PdfFontFactory.createFont("calibri/Calibri.ttf")).setFontSize(9.5f);//change path here
    ...
    protected class PageXofY implements IEventHandler {
        ...
        public void handleEvent(Event event) {
            ...
            try {
                ...
                Logger.getLogger(awinpd.class.getName()).log(Level.SEVERE, null, ex);
            }
